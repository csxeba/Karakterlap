...
class Tipus_Frame(Frame):
    def __init__(self, master, tipus, **kw):
        ...
        for nev in RES.kepzettsegek[tipus].keys():
            if "specializáció" in nev: continue
            if nev[0] == "!": nev = nev[1:]
...
class Kepz_toplevel(Toplevel):
        ...
        for rown, tul in enumerate(lst):
            ...
            for coln in range(3):
                if tul:
                    ...
                    if mod < 0: mod = 0
                    if   coln == 0: x = StringVar(value=lst[rown-2])
                    elif coln == 1: x = globz.kar.fo_tulajdonsagok[tul]
                    elif coln == 2: x = IntVar(value=mod)
                else: x = StringVar(value=" - ")
    ...
    def kepzettseg_fejlesztes(self, melyik, fok):
        print(melyik, fok, "TBD...")

    def get_gyenge_chain(self, kep):
        gyenge= ''
        if kep.kovetelmeny_k:
            if kep.kovetelmeny_k[1]: gyenge = gyenge + kep.kovetelmeny_k[1]
            else: gyenge = gyenge + 'Nincs'
        else:
            gyenge=gyenge+"Nincs"
        return gyenge

    def get_eros_chain(self, kep):
        eros = ''
        if kep.kovetelmeny_k:
            if kep.kovetelmeny_k[0]: eros = eros + kep.kovetelmeny_k[0]
            else: eros = eros + 'Nincs'
        else:
            eros = eros + "Nincs"
        return eros

    def get_tul_chain(self, kep):
        chain = ''
        if kep.kovetelmeny_t:
            for e in kep.kovetelmeny_t:
                chain = chain + e + ", "
            chain = chain[:-2]
        else: chain = chain + 'Nincs'
        return chain
